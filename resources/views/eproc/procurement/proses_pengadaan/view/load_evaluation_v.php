<?php foreach ($evaluation as $key => $value) {
 ...
 <tr>
  ...
  <td class="text-right">
    ...
    <?php } else { ?>
      <strong>
        <?php echo inttomoney($value['pte_technical_value']) ?>
      </strong>
    <?php } ?>
  </td>
  ...
  <td class="text-center">
   ...
   <?php } else { ?>
      <strong><?php echo inttomoney($value['pte_price_value']) ?></strong>
   <?php } ?>
   </td>
   <td>
   ...
   <td class="text-right">
     ...
     if(isset($first_price[$value['ptv_vendor_code']]['total_ppn'])) { 
      ...
      echo ($prep['ptp_submission_method'] > 0 AND ($value['adm'] != "Lulus" OR $value['pass'] != "Lulus")) ? "-" : inttomoney($value['amount']);
    } ?>
