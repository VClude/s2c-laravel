          ...
          <div class="ibox-content">
           ...
           <div class="form-group">
            ...
            <div class="col-sm-10">
             ...
             <input type="hidden" id="pr_number" value="<?php echo (isset($permintaan['pr_number'])) ? $permintaan['pr_number'] : ''; ?>">
           </div>
         </div>
   ...
   <?php $curval = (isset($permintaan['pr_tender_method'])) ? $metode_pengadaan[$permintaan["pr_tender_method"]] : '' ?>
        <div class="form-group">
          ...
          <div class="col-sm-4">
            ...
            <input type="hidden" id="metode_pengadaan_inp" name="metode_pengadaan_inp" value="<?php echo isset($permintaan['pr_tender_method']) ? $permintaan['pr_tender_method'] : ''; ?>">
          </div>
        </div>
...
<?php /*
        ...
        */ ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-5">
           <select class="form-control selectpicker" data-show-subtext="true" data-live-search="true" required name="lokasi_pengiriman_inp">
             <option value=""><?php echo lang('choose') ?></option>
             <?php foreach($del_point_list as $key => $val){
              $selected = ($val['dept_id'] == $curval) ? "selected" : ""; 
              $type = (!empty($val['dept_type'])) ? "Terminal" : "Divisi";
              ?>
        ...
        <?php $curval = (isset($permintaan['pr_type'])) ? $permintaan["pr_type"] : set_value("pr_type_inp"); ?>
        <div class="form-group">
          <label class="col-sm-2 control-label">Tipe PR *</label>
          <div class="col-sm-5">
           <select class="form-control" required name="pr_type_inp">
             <option value=""><?php echo lang('choose') ?></option>
             <?php foreach($pr_type as $key => $val){
              $selected = ($val['id'] == $curval) ? "selected" : ""; 
              ?>
              <option <?php echo $selected ?> value="<?php echo $val['id'] ?>"> <?php echo $val['pr_type'] ?> </option>
              <?php } ?>
            </select>
          </div>
        </div>
    ...
    function check_plan_tender(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $("#metode_pengadaan").text(mydata.metode);
          $("#metode_pengadaan_inp").val(mydata.ppm_tender_method);
        }
      });
    }
