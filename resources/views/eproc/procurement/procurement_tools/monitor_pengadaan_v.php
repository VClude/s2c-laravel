...
<script type="text/javascript">
  ...
  function tenderFormatter(value, row, index) {
    var tendermetode = "";

      if (row.ptp_tender_method == 0 && row.ptp_tender_method != null) {
        tendermetode += "Penunjukkan Langsung <br>";
      } else if (row.ptp_tender_method == 1) {
        tendermetode += "Pembelian Langsung <br>";
      } else if (row.ptp_tender_method == 2) {
        tendermetode += "Tender <br>";
      } else {
        tendermetode += "";
      }

      if (row.ptp_submission_method == 0 && row.ptp_submission_method != null) {
        tendermetode += "(1 Sampul)";
      } else if (row.ptp_submission_method == 1) {
        tendermetode += "(2 Sampul) ";
      } else if (row.ptp_submission_method == 2) {
        tendermetode += "(2 Tahap)";
      } else {
        tendermetode += "- ";
      }

      var metodepengadaan = (tendermetode) ? tendermetode : "-" ;
        return [
        metodepengadaan
      ].join('');
}
