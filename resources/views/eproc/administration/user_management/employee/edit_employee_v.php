<div class="wrapper wrapper-content">
  ...
  <script type="text/javascript">
      ...
      var emp = $('input[name="id"]').val();
      var hasUserAccess = <?php echo json_encode(array('status'=>$hasUserAccess)); ?>;
      var totalPosition = <?php echo json_encode(array('total'=>$totalPosition)); ?>;
      var empStatus = '<?php echo $data["status"]; ?>';
    ...
    function operateFormatter(value, row, index) {
      ...
      var total = totalPosition.total;

      if(total > 1 || hasUserAccess.status != true){
      return [
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
        if(totalPosition.total > 0){
            if(empStatus == '1'){
              $('select[name="status_inp"]').append('<option selected value="1">Aktif</option>');
              $('select[name="status_inp"]').append('<option value="0">Nonaktif</option>');
            }else{
              $('select[name="status_inp"]').append('<option value="1">Aktif</option>');
              $('select[name="status_inp"]').append('<option selected value="0">Nonaktif</option>');
            }
        }
    });
  </script>